...
pub unsafe fn init_esp_gdi() -> Result<(HDC, HDC, HBITMAP, HBRUSH, HBRUSH, HBRUSH), Box<String>> {
    ...
    let mut mem_bitmap = CreateCompatibleBitmap(hdc,
    ...
    match read_view_matrix(VIEW_MATRIX_ADDR) {
        ...
        Err(err) => {
            ...
            return Err(Box::new(format!("Error reading view matrix: {}",
                               err).to_string()));
        }
    };
    ...
    if font_handle.0.is_null() {
        ...
        return Err(Box::new(format!("Failed to add font from memory, error: {:?}",
                                    GetLastError()).to_string()));
    }

    // Return the handles and brushes
    Ok((hdc, mem_dc, mem_bitmap, red_brush, green_brush, background_brush))
}

pub unsafe fn read_game_data() -> Result<(usize, usize, usize, [f32; 16]), Box<String>> {
    // Read the local player address
    let local_player_addr = read_memory::<usize>(AC_CLIENT_EXE_HMODULE + LOCAL_PLAYER_OFFSET)
        .map_err(|err| Box::new(format!("Error reading local player address: {}", err)))?;



    // Read the number of players in the match
    let num_players_in_match = read_memory::<usize>(AC_CLIENT_EXE_HMODULE + NUMBER_OF_PLAYERS_IN_MATCH_OFFSET)
        .map_err(|err| Box::new(format!("Error reading number of players in match: {}", err)))? as usize;
...
)
{
    ...
    for i in 1..num_players_in_match {
        ...
        draw_border_box(mem_dc, box_brush_color, box_left, box_top, box_width, box_height, 5);

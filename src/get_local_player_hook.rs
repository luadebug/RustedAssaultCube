...
use crate::offsets::{
    ...
    CARBINE_COOLDOWN, GRENADES_COUNT, HEALTH_OFFSET_FROM_LOCAL_PLAYER, KNIFE_COOLDOWN,
    PISTOL_COOLDOWN, RIFLE_COOLDOWN, SHOTGUN_COOLDOWN, SNIPER_COOLDOWN, SUBMACHINEGUN_COOLDOWN,
};
...
#[inline(never)]
pub(crate) unsafe extern "cdecl" fn get_local_player_health(reg: *mut Registers, _: usize) {
    unsafe {
        if let Some(reg_val) = reg.as_ref() {
            ...
            if IS_NO_RELOAD.load(SeqCst) {
                ...
                local_player
                    .write_value(SUBMACHINEGUN_COOLDOWN, 0.0f32)
                    .ok();
            ...
            if IS_INVULNERABLE.load(SeqCst) {
                local_player
                    .write_value(HEALTH_OFFSET_FROM_LOCAL_PLAYER, 1337)
                    .ok();
                local_player
                    .write_value(ARMOR_OFFSET_FROM_LOCAL_PLAYER, 1337)
                    .ok();
            }
            ...
            if IS_INFINITE_AMMO.load(SeqCst) {
                ...
                local_player
                    .write_value(AMMO_IN_MAGAZINE_SUBMACHINEGUN, 40)
                    .ok();
...
pub fn setup_invul() {
    thread::spawn(move || {
        let pattern_mask = PatternMask::aob_to_pattern_mask("8B ? ? ? ? ? 83 F9 19");

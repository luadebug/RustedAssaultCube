...
pub unsafe fn esp_entrypoint() -> Result<(), Box<String>> {
    ...
    unsafe {
        ...
        loop {
            ...
            let num_players_in_match = match read_memory::<i32>(AC_CLIENT_EXE_HMODULE + NUMBER_OF_PLAYERS_IN_MATCH_OFFSET) {
                Ok(num) => num as usize,
                Err(err) => {
            ...
            if GAME_WINDOW_DIMENSIONS.width != get_window_dimensions(GAME_WINDOW_HANDLE).unwrap().width ||
                GAME_WINDOW_DIMENSIONS.height != get_window_dimensions(GAME_WINDOW_HANDLE).unwrap().height
            {
                ...
                if new_width != GAME_WINDOW_DIMENSIONS.width || new_height != GAME_WINDOW_DIMENSIONS.height {
            ...
            for i in 1..NUM_PLAYERS_IN_MATCH {
                ...
                if !world_to_screen(entity_position, &mut feet_screen_pos, VIEW_MATRIX, GAME_WINDOW_DIMENSIONS.width, GAME_WINDOW_DIMENSIONS.height) {
                    continue;
                }

                if !world_to_screen(entity_head_position, &mut head_screen_pos, VIEW_MATRIX, GAME_WINDOW_DIMENSIONS.width, GAME_WINDOW_DIMENSIONS.height) {
                    continue;
                }
                ...
                if IS_DRAW_FOV.load(SeqCst) {
                    draw_circle(hdc, (GAME_WINDOW_DIMENSIONS.width as f32 / 2.0, GAME_WINDOW_DIMENSIONS.height as f32 / 2.0), FOV.load(SeqCst) as f32, COLORREF(0x00FFFFFF));
                }
            ...
            if TransparentBlt(hdc, 0, 0, GAME_WINDOW_DIMENSIONS.width, GAME_WINDOW_DIMENSIONS.height,
                              mem_dc, 0, 0, GAME_WINDOW_DIMENSIONS.width, GAME_WINDOW_DIMENSIONS.height,
                              0x00000000) == FALSE {
